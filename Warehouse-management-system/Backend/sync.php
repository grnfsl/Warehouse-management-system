...
//$server_timestamp = $result->fetch_assoc()['UPDATE_TIME'];
//$sql = "SELECT num_operations FROM devices_id WHERE device_id='$device_id'";
//$result = $conn->query($sql);
//$num_operations = $result->fetch_assoc()['num_operations'];
//
//if ($device_num_operations != $num_operations) {
//    $sql = "UPDATE devices_id SET num_operations={$device_num_operations} WHERE device_id='{$device_id}'";
//    if ($conn->query($sql)) {
//        echo json_encode(array('error' => "0", 'message' => $device_num_operations - $num_rows . " new row(s) added"));
//    } else {
//        echo json_encode(array('error' => "1", 'message' => mysqli_error($conn) . $sql));
//    }
//
//
//    for ($x = 0; $x < $delta_num_rows; ++$x) {
//        if ($data_arr[$x]['operation'] == 'add') {
//            $sql1 = "select quantity from products where manufacturer_name='{$data_arr[$x]['manufacture']}' AND model_name='{$data_arr[$x]['model']}'";
//            $result = $conn->query($sql1);
//            $num_rows_q = mysqli_num_rows($result);
//            if ($num_rows_q == 0) {
//                $sql = "INSERT INTO products (manufacturer_name, model_name, price, quantity) VALUES ('{$data_arr[$x]['manufacture']}', '{$data_arr[$x]['model']}', '{$data_arr[$x]['price']}', '{$data_arr[$x]['quantity']}')";
//                if (!$conn->query($sql)) {
//                    echo json_encode(array('error' => "1", 'message' => $conn->error));
//                    die;
//                }
...
//            $num_rows_q = mysqli_num_rows($result);
//            if ($num_rows_q != 0) {
//                $sql = "DELETE FROM products WHERE manufacture_name='{$data_arr[$x]['manufacture']}' AND model_name='{$data_arr[$x]['model']}'";
//                if (!$conn->query($sql)) {
//                    echo json_encode(array('error' => "1", 'message' => $conn->error));
//                    die;
//                }
...
//                $sql = "UPDATE products SET quantity=$newValue WHERE manufacturer_name='$manufacturer' AND model_name='$model'";
//                if (!$conn->query($sql)) {
//                    echo json_encode(array('error' => "1", 'message' => $conn->error));
//                    die;
//                }
...
//    }
//} else {
//    echo json_encode(array('error' => "0", 'message' => "No rows added"));
//}
